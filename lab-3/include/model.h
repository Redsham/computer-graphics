...
public:
    ...
    vec4 vert(int i) const;                          // 0 <= i < nverts()
    vec4 vert(int iface, int nthvert) const;   // 0 <= iface <= nfaces(), 0 <= nthvert < 3
    vec4 normal(int iface, int nthvert) const; // normal coming from the "vn x y z" entries in the .obj file
    vec4 normal(const vec2 &uv) const;                     // normal vector from the normal map texture
    vec2 uv(int iface, int nthvert) const;     // uv coordinates of triangle corners
