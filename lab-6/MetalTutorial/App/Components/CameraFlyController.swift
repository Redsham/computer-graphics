...
final class CameraFlyController {
    ...
    init(view: MTKView,
         startPosition: simd_float3 = simd_float3(0.0, 4.0, 15.0),
         startYaw: Float = -.pi / 2.0,
         startPitch: Float = 0.0) {
        self.view = view
    ...
    func setPose(position: simd_float3, yaw: Float, pitch: Float) {
        self.position = position
        self.yaw = yaw
        self.pitch = max(-1.54, min(1.54, pitch))
    }
